...
get_psk_info(struct dtls_context_t *dtls_context,
  ...
  case DTLS_PSK_IDENTITY:
	...
	coap_log( LOG_WARNING, "cannot set psk -- hint too big\n" );
	goto error;
      }
    }
    result[0] = 0;
    psk_len = coap_context->get_client_psk( coap_session, buf, (char*)result, (unsigned)result_length, psk, (unsigned)sizeof( psk ) );
    if ( !psk_len ) {
