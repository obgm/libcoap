...
#define BUFSIZE 100
	...
	if (uri.scheme == COAP_URI_SCHEME_COAP) {
		session = coap_new_client_session3(ctx, NULL, &dst, COAP_PROTO_UDP, NULL, NULL, NULL);
		...
		session = coap_new_client_session_psk3(ctx, NULL, &dst, COAP_PROTO_DTLS, &dtls_psk, NULL, NULL, NULL);
